  ...
  conf.json.toOption match {
    ...
    case Some(jsonFile) => {
      val answers = ujson.Obj(
        ...
        "R.1" -> ujson.Obj(
          "PredUser1Item1" -> ujson.Num(knnPredict(data++personal, 300, 1, 1)) // Prediction for user 1 of item 1
        ),
        ...
        "R.2" -> recommend(data, personal, 300, 944).map(x => ujson.Arr(x._1, movieNames(x._1), x._2))
       )
