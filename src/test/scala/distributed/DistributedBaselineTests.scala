...
class DistributedBaselineTests extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("Compute global average")                           
    { assert(within(distr_globalAvgRating(train2),       3.5264, 0.0001)) }
  test("Compute user 1 average")                           
    { assert(within(distr_userAvgRating  (train2, 1),    3.6330, 0.0001)) }
  test("Compute item 1 average")                           
    { assert(within(distr_itemAvgRating  (train2, 1),    3.8882, 0.0001)) }
  test("Compute item 1 average deviation")                 
    { assert(within(distr_itemAvgDev     (train2, 1),    0.3027, 0.0001)) }
  test("Compute baseline prediction for user 1 on item 1") 
    { assert(within(distr_baselineRating (train2, 1, 1), 4.0468, 0.0001)) }
  ...
  test("MAE on all four non-personalized methods on data/ml-100k/u2.base and data/ml-100k/u2.test") {
    ...
    assert(within(calcMAE(distr_predictorItemAvg),   0.8206, 0.0001))
    assert(within(calcMAE(distr_predictorBaseline),  0.7604, 0.0001))
  }
}
